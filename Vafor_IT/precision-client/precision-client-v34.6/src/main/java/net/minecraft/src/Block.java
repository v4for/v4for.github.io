...
public class Block {
    ...
    public static final void initNoiseField(int cx, int cz) {
        if (noiseChunkX != cx || noiseChunkZ != cz) {
            noiseChunkX = cx;
            noiseChunkZ = cz;
            initNoiseField0(cx, cz);
        }
    }

    public static void initNoiseField0(int cx, int cz) {
        double scale = 0.05D;
        for (int i = 0; i < grassNoiseArray.length; ++i) {
            grassNoiseArray[i] = 0.0D;
        }
        grassNoise.populateNoiseArray(grassNoiseArray, cx * 16.0D * scale, 16.0D * scale, cz * 16.0D * scale, 16, 1, 16, scale, 1.0D, scale, 1.0D);
    }
    ...
    public static final BlockRedstoneRepeater redstoneRepeaterActive = (BlockRedstoneRepeater) (new BlockRedstoneRepeater(94, true)).setHardness(0.0F).setLightValue(0.625F).setStepSound(soundWoodFootstep).setUnlocalizedName("diode")
            .disableStats();
    ...
    public static final Block endPortalFrame = (new BlockEndPortalFrame(120)).setStepSound(soundGlassFootstep).setLightValue(0.125F).setHardness(-1.0F).setUnlocalizedName("endPortalFrame").setResistance(6000000.0F)
