...
public class GameSettings {
    ...
    public void setOptionValue(EnumOptions par1EnumOptions, int par2) {
        ...
        /* Precision Client settings */
        if (par1EnumOptions == EnumOptions.DYNAMIC_FOV) {
            this.dynamicFOV = !this.dynamicFOV;
        }

        if (par1EnumOptions == EnumOptions.FULLBRIGHT) {
    ...
    public void loadOptions() {
        ...
        if (!yee.hasNoTags()) {
            ...
            for (int var4 = 0; var4 < this.keyBindings.length; ++var4) {
                if (yee.hasKey(keyBindings[var4].keyDescription))
                    this.keyBindings[var4].keyCode = yee.getInteger(keyBindings[var4].keyDescription);
            }
