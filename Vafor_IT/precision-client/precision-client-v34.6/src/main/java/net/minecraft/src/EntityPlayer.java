...
public abstract class EntityPlayer extends EntityLiving implements ICommandSender {
    ...
    /**
     * The chunk coordinates of the bed the player is in (null if player isn't in a
     * bed).
     */
    ...
    public int getBrightnessForRender(float par1) {
        return DefaultSkinRenderer.getPlayerRenderer(this) == 21 ? 15728880 : super.getBrightnessForRender(par1);
    }
    ...
    public void onUpdate() {
        ...
        if (!(this.equals(Minecraft.getMinecraft().renderViewEntity) && Minecraft.getMinecraft().gameSettings.thirdPersonView == 0)) {
            int i = DefaultSkinRenderer.getPlayerRenderer(this);
            if (i == 21) {
                this.worldObj.spawnParticle("largesmoke", this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY - this.yOffset + this.rand.nextDouble() * (double) this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, 0.0D, 0.0D, 0.0D);
            } else if (i == 19) {
                for (int j = 0; j < 2; ++j) {
                    this.worldObj.spawnParticle("portal", this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width, this.posY - this.yOffset + this.rand.nextDouble() * 2.5D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width, (this.rand.nextDouble() - 0.5D) * 2.0D, -this.rand.nextDouble(), (this.rand.nextDouble() - 0.5D) * 2.0D);
                }
            }
        }
        ...
        if (par1Entity.canAttackWithItem()) {
            if (!par1Entity.func_85031_j(this)) {
                ...
                if (var2 > 0 || var4 > 0) {
                    ...
                    if (var8) {
                        if (var3 > 0) {
                            par1Entity.addVelocity(-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F) * (float) var3 * 0.5F, 0.1D,
