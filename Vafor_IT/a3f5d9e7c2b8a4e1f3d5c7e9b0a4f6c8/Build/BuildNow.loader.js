            ...
            if (!progress.started) {
                ...
                if (r && !r.lengthComputable) {
                    if (r.target) {
                        ...
                        if (r.target.getAllResponseHeaders().indexOf(headerName) >= 0) {
                            var ogSize = parseInt(r.target.getResponseHeader(headerName), 10) || 0;
                            totalEstimate = ogSize
                        } else {
                            var length = parseInt(r.target.getResponseHeader("Content-Length"), 10) || 0;
                            totalEstimate = length * 1.5
                        }
                    }
                }
                progress.started = true;
                progress.lengthComputable = r.lengthComputable;
                progress.total = r.total ? r.total : totalEstimate
            }
            progress.loaded = r.loaded || 0;
            if (r.type == "load") {
